...
Aşağıdaki örnekte görüldüğü üzere 	```Animal``` ana sınıfına ait ```sound``` metodu çağrılıyor. Hayvan ana sınıfından domuz, kedi, köpek, kuş gibi sınıflar türetilebilir ve onlar hayvan sesini kendi içlerinde farklılaştırabilir, ```sound``` metoduna farklı görevler yükleyebilir. Böylece nesne yönelimli programlanın çok biçimlilik özelliği, tek bir eylemi farklı şekillerde gerçekleştirmemizi sağlar. 
<br />
